...
class ResultRanking(object):
    ...
    def __init__(self, columns, entries=None):
        ...
        entrties: list(flowserv.model.ranking.RunResult), optional
    ...
    def get(self, index):
        ...
        Returns
        -------
        flowserv.model.ranking.RunResult
        """
...
def create_result_table(con, benchmark_id, schema, commit_changes=True):
    ...
    benchmark_id: string
        Unique benchmark identifier
    schema: flowserv.template.schema.ResultSchema
...
def insert_run_results(con, run_id, files, commit_changes=True):
    ...
    run_id: string
        Unique run identifier
    files: dict(flowserv.model.workflow.resource.FSObject)
    ...
    if row['result_schema'] is not None:
        ...
        f = files.get(schema.result_file)
        if f is None:
            return
