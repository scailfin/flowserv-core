...
"""Unit tests for validation of template parameter declarations."""
...
def test_maximal_declaration():
    ...
    assert len(values) == 3
    validate_value(values[0], 1, '1', False)
    validate_value(values[1], 2, 'Two', False)
    validate_value(values[2], 3, 'THREE', True)
    # Ensure that the returned dictionary is valid with respect to the
    # parameter schema declaration.
    pd.validate_parameter(p)


def test_minimal_declaration():
...
def validate_value(obj, value, name, is_default):
