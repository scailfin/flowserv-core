...
def test_delete_group_file_local(local_service, hello_world, tmpdir):
    ...
    with local_service(config=config, user_id='UNKNNOWN') as api:
...
def test_list_group_files_local(local_service, hello_world, tmpdir):
    ...
    with local_service(config=config, user_id='UNKNOWN') as api:
        with pytest.raises(err.UnauthorizedAccessError):
...
def test_upload_group_file_local(local_service, hello_world, tmpdir):
    ...
    for uid in [user_id, None]:
        with local_service(config=config, user_id=uid) as api:
